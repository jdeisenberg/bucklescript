  ...
  *)
  ...
  | Pctconst ct ->
    begin
      match ct with
      | Big_endian ->
        if Sys.big_endian then  E.js_true
        else E.js_false
      | Word_size ->
        E.small_int  Sys.word_size
      | Ostype_unix ->
        if Sys.unix then E.js_true else E.js_false
      | Ostype_win32 ->
        if Sys.win32 then E.js_true else E.js_false
      | Ostype_cygwin ->
        if Sys.cygwin then E.js_true else E.js_false
    end
  (* | Psetglobal _  ->  *)
